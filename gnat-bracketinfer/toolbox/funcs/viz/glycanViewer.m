...
isGlycanStruct = isa(glycanObj,'GlycanStruct');
isGlycanSpecies = isa(glycanObj,'GlycanSpecies');
isGlycanJava = isa(glycanObj,'org.eurocarbdb.application.glycanbuilder.Glycan');

if ~(isGlycanStruct||isGlycanJava||isGlycanSpecies)
    error('MATLAB:GNAT:WRONGINPUT','WrongInputType');
end;
...
elseif(isGlycanSpecies)
    glycanStruct = structMat2Java(glycanObj.glycanStruct);
else
