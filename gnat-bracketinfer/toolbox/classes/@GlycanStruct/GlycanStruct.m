classdef GlycanStruct < handle
    ...
    methods
        ...
        function obj=removeBracket(obj)
            obj.bracket = [];
            obj.resetjava;
        end
        
        function obj=moveResiduesfrombracket2root(obj,...
            bracketresiduetoremove,numresidues,rootresiduetoattach)
             if(numresidues==0)
                 return
             end
        
              isresidueremoved = obj.removeResidueObj(...
                  bracketresiduetoremove);
              if(isresidueremoved)
                  % check if any residue remains in the bracket
                  nredendresiduetobeextended = obj.findithgenchildren(...
                  bracketresiduetoremove,numresidues);
              
                  if(~isempty(nredendresiduetobeextended))
                    obj.removeResidueObj(nredendresiduetobeextended);  
                    obj.addResiduesToBracket(nredendresiduetobeextended);
                  end
                  
                  % add any residue to root
                  obj.addResidue(rootresiduetoattach,bracketresiduetoremove);
              end
        end
        
        function ithgenchildren = findithgenchildren(obj,residue,ith,varargin)
            if(length(varargin)==1)
                ithgenchildren = varargin{1};
            else
                ithgenchildren = cell(0,1);
            end
            
            if(ith<=0)
                ithgenchildren(end+1)=residue;
                return
            end
            
            i=1;
            while(i<=ith)&&(~isempty(residue.getLinkageChildren))
                for j = 1 :length(residue.getLinkageChildren)
                    jthchild       = residue.getLinkageChildren(j,1).getChild;
                    ithgenchildren = findithgenchildren(obj,jthchild,ith-1,ithgenchildren);
                end
            end
        end
        ...
        function nonredEndResidues = getNonRedEndResidue(obj,varargin)
            ...
            elseif(length(varargin)==1)
                if(strcmpi(varargin{1},'bracket')||strcmpi(varargin{1},'root'))
                    allResidues        = getAllResidues(obj,varargin{1});
                    nonredEndResidues  = selectTerminalResidue(obj,allResidues);
                else
                    error('MATLAB:GNAT:ERRORINPUT','INCORRECT INPUT TYPE');
                end
            else
                error('MATLAB:GNAT:WRONGNUMBERINPUT','WRONG NUMBER OF INPUTS');
            end
        end
        
        function nredEndResidues= selectTerminalResidue(obj,allResidues)
            % SELECTTERMINALRESIDUE select residues on the non-reducing end
        ...
        function  resVector  = getAllResidues(obj,varargin)
            ...
            elseif(length(varargin)==1)
                if(strcmpi(varargin,'root'))
                    resVector  =  getResidues(obj.root,resVector);
                elseif(strcmpi(varargin,'bracket'))
                    resVector   = getResidues(obj.bracket,resVector);
                else
                    error('MATLAB:GNAT:ERRORINPUTSTRING','WRONG INPUT STRING');
                end
            else
                error('MATLAB:GNAT:ERRORINPUTNUMBER','WRONG INPUT NUMBER');
            end
            ...
            %inner sub-function
             function resVector = getResidues(residue,resVector)
                    ...
                    for i=1: length(childLinkageList)
...
%             isStructEqual = strcmp(obj.toString,obj2.toString);
%         end
        ...
        function ismonocompsame=compContain(obj,obj2)
            %compcompare:Compare compsotions between two glycans
            % Input: 
            %     obj: GlycanStruct object 1
            %     obj2: GlycanStruct object 2
            %
            % Output: 
            %    ismonocompsame: 
            %                    1 -- same monosaccharides and same number
            %                    2 -- obj1 composition greater than obj2
            %                           composition
            %                    0 -- the other conditions
            %See also getCompositionString.
            
            ismonocompsame = 2;
            comps1=obj.getComposition;
            comps2=obj2.getComposition;
            comps1fieldnames = fieldnames(comps1);
            comps2fieldnames = fieldnames(comps2);
            
            if(isequal(comps1,comps2))
                ismonocompsame = 1;
            elseif(length(comps1fieldnames)<length(comps2fieldnames))
                ismonocompsame = 0;
            else
                for i = 1 : length(comps2fieldnames)
                    ithfield = comps2fieldnames{i};
                    if(isempty(strcmp(ithfield,comps1fieldnames)))
                        ismonocompsame = 0;
                        return
                    else
                        if(comps1.(ithfield)<comps2.(ithfield))
                            ismonocompsame = 0;
                            return
                        end
                    end
    ...
    methods
        ...
        isStructEqual       = equalStruct(obj,obj2)
    end
    ...
    methods
        ...
        isResidueDeleted  = removeResidueObj(obj,residuetoremove,varargin)
        residueToRootChar = getresiduetoroot(obj,residue,varargin)
        isstructcontained = bracketContain(obj,obj2);
        branches          = getBranch(obj,varargin);
        branches          = getComplexNGlycanBranches(obj);
        branches          = getBranches(obj);
    end
end
