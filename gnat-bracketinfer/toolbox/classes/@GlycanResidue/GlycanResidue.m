...
%             end
        ...
        function  isresidueequal = subtreequal(obj,obj2)
          %fprintf(1,'obj1: %s',obj.residueType.name)
          % fprintf(1,'\tobj2: %s\n',obj2.residueType.name)
          
%           if(strcmpi(obj.residueType.name,'Gal'))
%               disp('debuging point');
%           end
          ...
          for i = 1 : length(children1)
              ...
              for j = 1 : length(children2)  
                 if(isempty(storedcheckpos))
                     notcheckedbefore  = true;
                 else
                     notcheckedbefore = isempty(find(~(storedcheckpos-j), 1));                    
                 end  
                 
                 if(notcheckedbefore)    
                      jthchild     = children2(j,1);                  
                      ischildequal = ithchild.subtreequal(jthchild);
                      if(ischildequal) 
