classdef GlycanNetModel < handle
    ...
    methods
        function obj = GlycanNetModel(varargin)
            ...
            if(nargin>0)
                ...
                elseif(nargin==3)
                    obj.compartment   =  varargin{1};   %  compartment; CellArrayList
    ...
    methods (Static)
        function modelobj=loadmat(matfilename)
            pathstruct=load(matfilename);
            p = fieldnames(pathstruct);
            if(length(p)~=1)
                error('MATLAB:GNAT:WRONGINPUT','wrong variable stored');
            end
            modelobj = pathstruct.(p{1});
            if(isa(modelobj,'GlycanNetModel'))
                modelobj.glycanpathway.resetjava;
            else
                error('MATLAB:GNAT:WRONGINPUT','wrong variable stored');
            end
        end
